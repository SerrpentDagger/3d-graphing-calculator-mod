...
////////////////////////////////////////////////////////////////////// GL Call List / Tessellator combo
...
public class FastTESRGC extends FastTESR<TileGCBase>
{
			...
			BufferBuilder buffer)
	{
		...
		else
		{
			...
			if (!transparent)
			{
				GlStateManager.translate(pos.getX() - TileEntityRendererDispatcher.staticPlayerX, pos.getY() - TileEntityRendererDispatcher.staticPlayerY,
	...
	public void renderGraph(BufferBuilder buffer)
	{
	...
	private void vert(BufferBuilder buffer, Vec3d vec, double u, double v, int j, int k)
	{
		...
		if (doSlope)
		{
			...
			Col col2 = shiftHue(col, 300 * sRatio);
			buffer.pos(vec.x, vec.y, vec.z).color(col2.r, col2.g, col2.b, a).tex(u, v).lightmap(80, lightmap).endVertex();
		}
		else
		{
			buffer.pos(vec.x, vec.y, vec.z).color(r, g, b, a).tex(u, v).lightmap(80, lightmap).endVertex();
		}
	}
	...
	public void deleteVertexData(TileGCBase te)
	{
		...
		renderIndex = 0;
		orderedRender = ArrayUtils.removeElement(orderedRender, te);
	}
	...
	public void sort()
	{
		...
		for (int i = 1; i < orderedRender.length; i++)
		{
			...
			Vec3d pos1 = new Vec3d(orderedRender[i].getPos()).addVector(te1.translation[0], te1.translation[1], te1.translation[2]);
			Vec3d pos2 = new Vec3d(orderedRender[i - 1].getPos()).addVector(te2.translation[0], te2.translation[1], te2.translation[2]);
...
public class FastTESRGC extends FastTESR<TileGCBase>
{
			...
			BufferBuilder buffer)
	{
		...
		{
			transparent = orderedRender[renderIndex].shouldRenderInPass(1);
			Minecraft.getMinecraft().renderEngine.bindTexture(TextureMap.LOCATION_BLOCKS_TEXTURE);
			RenderHelper.disableStandardItemLighting();
			GlStateManager.enableBlend();
			GlStateManager.blendFunc(org.lwjgl.opengl.GL11.GL_SRC_ALPHA, org.lwjgl.opengl.GL11.GL_ONE_MINUS_SRC_ALPHA);
			...
			if (!sorted[renderIndex] && transparent)
			{
				Vec3d cameraPos = ActiveRenderInfo.getCameraPosition();
				Vec3d translation = Minecraft.getMinecraft().player.getPositionEyes(partialTicks);
				translation = new Vec3d(cameraPos.x + translation.x - pos.getX(), cameraPos.y + translation.y - pos.getY(), cameraPos.z + translation.z - pos.getZ());
