...
public class TileGCSphericalTheta extends TileGCBase
{
	public TileGCSphericalTheta()
	{
	...
	public void genMesh()
	{
		...
		for (int j = 0; j < vertexArray.length; j++)
		{
			...
			{
				temp.setTo(vertexArray[j][k].x, vertexArray[j][k].z, (Math.PI / 2) - vertexArray[j][k].y, Alt3d.SPHERICAL);
				scaleTrans(temp);
